...
class ScheduleManager:
    ...
    def init_db(self):
        with sqlite3.connect("schedule_bot.db") as conn:
            ...
            try:
                conn.execute('ALTER TABLE user_settings ADD COLUMN auto_topic_id INTEGER')
    ...
    def set_auto_chat(self, user_id, chat_id, topic_id=None):
        with sqlite3.connect("schedule_bot.db") as conn:
            conn.execute('''
                INSERT OR REPLACE INTO user_settings 
                (user_id, auto_chat_id, auto_topic_id, auto_enabled)
                VALUES (?, ?, ?, 1)
    ...
    def disable_auto(self, user_id):
        with sqlite3.connect("schedule_bot.db") as conn:
            conn.execute(
                'UPDATE user_settings SET auto_enabled = 0 WHERE user_id = ?',
                (user_id,)
            )
    ...
    def get_auto_chat(self, user_id):
        with sqlite3.connect("schedule_bot.db") as conn:
            cur = conn.execute(
                'SELECT auto_chat_id, auto_topic_id, auto_enabled FROM user_settings WHERE user_id = ?',
                (user_id,)
            )
            row = cur.fetchone()
            if row:
    ...
    def get_all_auto_chats(self):
        with sqlite3.connect("schedule_bot.db") as conn:            cur = conn.execute(
                'SELECT user_id, auto_chat_id, auto_topic_id FROM user_settings WHERE auto_enabled = 1'
            )
            return cur.fetchall()
...
def get_last_lesson_end_time(day_name, week_type):
    lessons = SCHEDULE.get(week_type, {}).get(day_name, [])
    if not lessons:
        return None
    end_times = []
...
def with_cleanup(handler):    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        "🤖 Бот расписания СибСАУ\n\n"
        "📋 Команды:\n"
        "/today — расписание на сегодня\n"
        "/tomorrow — расписание на завтра\n"
        "/week — расписание на всю неделю\n"
        "/day <день> — конкретный день (напр. /day вторник)\n"
        "/setchat [chat_id] [topic_id] — настроить автоотправку после последней пары\n"
...
async def day_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    arg = " ".join(context.args).lower()    day = mapping.get(arg)

    if not day:
        msg = await update.message.reply_text(
            "❌ Укажите день недели:\n/day понедельник" + FOOTER_LINK,
            parse_mode='HTML'
        )
    else:
...
async def week_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for eng in ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]:
        lessons = SCHEDULE.get(week_type, {}).get(eng, [])
...
@with_cleanupasync def setchat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Настройка чата для автоотправки: /setchat [chat_id] [topic_id]"""
    ...
    if update.effective_chat.type == "private":
        ...
        if context.args:
            ...
            if len(context.args) > 1 and context.args[1].isdigit():
                target_topic_id = int(context.args[1])
    else:
        if not context.args or not context.args[0].lstrip('-').isdigit():
            msg = await update.message.reply_text(
                "⚙️ Настройка автоотправки:\n"
                "• В ЛС: просто <code>/setchat</code>\n"
                "• В группе: <code>/setchat &lt;chat_id&gt; [topic_id]</code>\n"
                "• ID чата: @getidsbot\n"
                "• Topic ID — номер темы в форуме (опционально)" + FOOTER_LINK,
                parse_mode='HTML'
            )
            manager.save_message(update.effective_chat.id, user_id, msg.message_id)
            return
        target_chat_id = int(context.args[0])
        target_topic_id = int(context.args[1]) if len(context.args) > 1 and context.args[1].isdigit() else None

    manager.set_auto_chat(user_id, target_chat_id, target_topic_id)

    topic_info = f"\n🧵 Топик: <code>{target_topic_id}</code>" if target_topic_id else ""
    msg = await update.message.reply_text(
        f"✅ Автоотправка настроена!\n"
        f"🆔 Чат: <code>{target_chat_id}</code>{topic_info}\n"
...
async def send_tomorrow_schedule(bot, chat_id, week_type_tomorrow, topic_id=None):
    """Отправляет расписание на завтра в указанный чат/топик"""
    ...
    await bot.send_message(
        ...
        text=schedule_text,
        parse_mode='HTML',
        message_thread_id=topic_id  # ← отправка в конкретный топик форума
    )
...
async def schedule_auto_send(app: Application):
    ...
    if datetime.datetime.now(KRASNOYARSK_TZ).date() != today:
        logging.warning("🔄 Дата изменилась во время ожидания, пропускаем отправку")        return

    manager = ScheduleManager()
    sent_count = 0

    for user_id, target_chat_id, target_topic_id in manager.get_all_auto_chats():
        try:
            await send_tomorrow_schedule(app.bot, target_chat_id, tomorrow_week, target_topic_id)
            logging.info(f"✅ Отправлено пользователю {user_id} в чат {target_chat_id}" + (f" (топик {target_topic_id})" if target_topic_id else ""))
            sent_count += 1
        except Forbidden:
